...
const HospitalListTable = () => {
	...
	const handleDeleteSelected = async () => {
		...
		if (
			window.confirm(
				`Are you sure you want to delete ${selectedHospitals.length} hospital(s)?`
			)
		) {
			try {
				await Promise.all(
					selectedHospitals.map((id) => HospitalAPI.deleteHospital(id))
				);
				setSelectedHospitals([]);
				fetchHospitals();
			} catch (err) {
											...
											<td>
												<div className="d-flex gap-2">
													...
													<button
														...
														onClick={() =>
															handleDeleteHospital(hospital._id, hospital.name)
														}
						...
						{hospitals.length === 0 && !loading && (
							<div className="text-center py-5">
								...
								<p className="text-muted">
									Try adjusting your search criteria or create a new hospital.
								</p>
								<button
									className="btn btn-primary"
									onClick={() => navigate("/hospitals/create")}
