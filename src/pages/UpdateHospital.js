...
const UpdateHospital = () => {
  ...
  const fetchHospitalData = async () => {
    try {
      setLoading(true);
      const data = await HospitalAPI.getHospitalById(id);
      const hospital = data.hospital;
      
      // Convert single values to arrays for multi-select fields
      const clinicType = Array.isArray(hospital.clinicType) ? hospital.clinicType : 
        (hospital.clinicType ? [hospital.clinicType] : []);
      const typeOfCare = Array.isArray(hospital.typeOfCare) ? hospital.typeOfCare : 
        (hospital.typeOfCare ? [hospital.typeOfCare] : []);
      const ageGroup = Array.isArray(hospital.ageGroup) ? hospital.ageGroup : 
        (hospital.ageGroup ? [hospital.ageGroup] : []);
      ...
      setFormData({
        ...
        googleReviewsRating: hospital.googleReviewsRating || '',
        userScore: hospital.userScore || '',
        foundedYear: hospital.foundedYear || '',
        totalPatients: hospital.totalPatients || '',
        totalUnits: hospital.totalUnits || '',
        sizeAndCapacity: {
          operationTheatres: hospital.sizeAndCapacity?.operationTheatres || '',
          icus: hospital.sizeAndCapacity?.icus || '',
          patientBeds: hospital.sizeAndCapacity?.patientBeds || ''
        },
        clinicType: clinicType,
        typeOfCare: typeOfCare,
        ageGroup: ageGroup,
        about: hospital.about || '',
        primaryFocus: hospital.primaryFocus || '',
        departments: hospital.departments && hospital.departments.length > 0 ? 
          hospital.departments : [{ name: '', doctors: [{ name: '', specialties: [] }] }],
        featureAndFacts: Array.isArray(hospital.featureAndFacts) && hospital.featureAndFacts.length > 0 ? hospital.featureAndFacts : [{ title: '', description: '' }],
        faq: Array.isArray(hospital.faq) && hospital.faq.length > 0 ? hospital.faq : [{ question: '', answer: '' }],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await HospitalAPI.updateHospital(id, formDataToSend);
      navigate('/hospitals/table');
    } catch (err) {
      setError(err.message);
    } finally {
      setSaving(false);
      ...
      <div className="bg-white">
        <div className="container py-4">
          <div className="d-flex justify-content-between align-items-center mb-4">
            <h2 className="h3 fw-bold text-black mb-0">Update Hospital</h2>
            <button 
              className="btn btn-outline-secondary"
              onClick={() => navigate('/hospitals/table')}
            >
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="card border-0 shadow-sm mb-4">
                ...
                <div className="card-body">
                  <div className="row">
                    <div className="col-md-6 mb-3">
                      <label className="form-label">Update Main Image</label>
                      <input
                        type="file"
                        className="form-control"
                        name="mainImage"
                    ...
                    <div className="col-md-6 mb-3">
                      ...
                      <input
                        ...
                        onChange={handleFileChange}
                      />
                      <small className="text-muted">These will be added to existing gallery</small>
                    </div>
                  </div>
                </div>
              </div>
